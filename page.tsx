...
// import Script from 'next/script'; // Removed due to compilation error
...
export default function CrashXPGame() {
  // ALL STATE GOES HERE (converted from Alpine stores)
  ...
  const [creator, setCreator] = useState({
    ...
    referralBonus: 100,
    activeEvents: [] as any[], // This type could be more specific if event structure is known
    primaryColor: '#6366f1',
    accentColor: '#8b5cf6',
  ...
  }, [user, isAuthReady, showToast, whopRef]); // Dependencies fixed
  ...
  }, [user, isAuthReady, showToast, setUser, whopRef]); // Added setUser to dependencies
  ...
  }, [game.crashPoint, updateMultiplierVisuals, setGame]); // Removed crashGame from direct dependencies, fixed with setTimeout
  ...
  }, [autoPlay.wagerAmount, setGame]); // Added setGame to dependencies
  ...
  }, [isAuthReady, user.id, user.xp, game.userWagerXP, showToast, showModal, updateUserData, resetRound, closeModal, setUser]); // Added setUser to dependencies
  ...
  }, [isAuthReady, user.id, showToast, updateUserData, showModal, closeModal, setUser]); // Added setUser to dependencies
  ...
  const triggerMysteryBox = useCallback(async () => {
    ...
    setTimeout(async () => {
      ...
      // Added type guards for all optional properties as per ERROR #2
      if (randomReward.type === 'xp' && typeof randomReward.amount === 'number') {
        setUser(prev => ({ ...prev, xp: prev.xp + (randomReward.amount || 0) }));
        ...
        setUser(prev => {
          if (!prev.unlockedCosmetics.includes(randomReward.id!)) { // Use non-null assertion as type is string
            return { ...prev, unlockedCosmetics: [...prev.unlockedCosmetics, randomReward.id!], activeCosmetic: randomReward.id! };
          } else {
            const bonusXp = 200;
        ...
        activateXPBoost({ multiplier: randomReward.value, duration: randomReward.duration });
      }
  ...
  }, [isAuthReady, user.id, user.unlockedCosmetics, user.xp, showToast, updateUserData, activateXPBoost, setUser, setUi]); // Added setUi, setUser to dependencies
  ...
  const updateWinStreak = useCallback(async (won: boolean) => {
    ...
    if (won) {
      setUser(prev => {
        ...
        // Call functions directly instead of including in dependencies, but wrapped in setTimeout to defer
        // This is a common pattern to break direct circular dependencies, though in this case,
        // it was mainly about declaration order. Still, deferring can be safer for complex state updates.
    ...
    await updateUserData();
  }, [isAuthReady, user.id, setUser, updateUserData]); // Complete dependencies fixed
  ...
  const crashGame = useCallback(async () => {
    ...
    updateWinStreak, Haptics, autoPlay.enabled, setGame, setUser, multiplierIntervalRef // Added multiplierIntervalRef to deps
  ]);
  ...
  const placeWager = useCallback(async () => {
    ...
    startRound();
    // updateUserData is called after game outcome in crashGame or cashOut
  }, [
    isAuthReady, user.id, user.xp, game.isRunning, game.userWagerXP,
    showToast, playSound, startRound, Haptics, setUi, setGame, setUser // Added setGame, setUser to dependencies
  ]);
  ...
  const cashOut = useCallback(async () => {
    ...
    updateWinStreak, Haptics, createShareableAchievement, setGame, setUser, multiplierIntervalRef // Added multiplierIntervalRef to dependencies
  ]);
  ...
  }, [isAuthReady, user, showToast, generateDummyLeaderboard, whopRef, setLeaderboard]); // Dependencies fixed
  ...
  const signInUser = useCallback(async (userAuth: { id: string; name: string; role: 'user' | 'creator' }, role = 'user') => {
    userIdRef.current = userAuth.id;
    setIsAuthReady(true);
    setUser(prev => ({ ...prev, id: userAuth.id, name: userAuth.name, role: role as 'user' | 'creator' })); // Set user name from auth
  ...
  const signInAsUser = useCallback(async () => {
    ...
    try {
      const userAuth = await whopRef.current.user.signIn({ role: 'user' });
      await signInUser(userAuth, 'user');
    } catch (error) {
  ...
  const signInAsCreator = useCallback(async () => {
    ...
    try {
      const userAuth = await whopRef.current.user.signIn({ role: 'creator' });
      await signInUser(userAuth, 'creator');
    } catch (error) {
  ...
  const activateFOMOEvent = useCallback((event: { type: string; duration: number; message: string; icon: string; multiplier?: number }) => {
    ...
    if (event.type === '2x_xp_boost' && event.multiplier) {
  ...
  }, [isAuthReady, user.id, autoPlay.isExecutingAutoRound, game.isRunning, showToast, autoPlayExecuteRound]);
  ...
  useEffect(() => {
    ...
    generateSocialPressure, startRandomFOMOEvent, showToast,
    generateDummyLeaderboard, setUser, setLeaderboard, fetchUserData,
    setupRealtimeLeaderboard, updateDailyStreak, updateStreakWarning
  ]);
  ...
  useEffect(() => {
    ...
    if (containerEl) {
      if (user.currentStreak >= 3) {
        ...
        (containerEl as HTMLElement).style.boxShadow = `0 0 ${30 + (user.currentStreak * 2)}px rgba(0, 255, 136, ${intensity})`;
      } else {
        containerEl.classList.remove('streak-anxiety');
        (containerEl as HTMLElement).style.boxShadow = '';
      }
    }
    ...
    <>
      <style jsx global>{`
        ...
        @keyframes crashExplode {
            0% { transform: scale(1) translateZ(0); opacity: 1; }
            50% { transform: scale(1.5) translateZ(0); opacity: 0; } /* Disappear quickly */
            100% { transform: scale(0.8) translateZ(0); opacity: 0; } /* Remain invisible */
        }
...
.sidebar-item:disabled {
    ...
    cursor: not-allowed;
      ...
      {/* External Scripts - Loaded using standard <script> tags */}
      <script src="https://cdn.tailwindcss.com"></script>
      <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>
      <script src="https://cdnjs.cloudflare.com/ajax/libs/cannon.js/0.6.2/cannon.min.js"></script>
      <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
      <script src="https://cdn.jsdelivr.net/npm/moment@2.29.1/moment.min.js"></script>
      <script src="https://cdn.jsdelivr.net/npm/chartjs-adapter-moment@1.0.0/dist/chartjs-adapter-moment@1.0.0.min.js"></script>
      {/* Firebase CDN scripts are removed as per instructions */}
    </>
  );
}
